...
=== "リレーノード"
    ...
    /usr/local/bin/cardano-node +RTS -N -A16m -qg -qb -T -S -RTS run --topology \${TOPOLOGY} --database-path \${DB_PATH} --socket-path \${SOCKET_PATH} --host-addr \${HOSTADDR} --port \${PORT} --config \${CONFIG}
    EOF
    ```
