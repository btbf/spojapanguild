...
{% hint style="success" %}
このマニュアルは、カルダノノードv1.32.1に対応しています。  
[ドキュメント更新情報はこちら](README.md)  
最終更新日：2021年12月29日の時点guide version 8.1
{% endhint %}
...
{% hint style="info" %}
Cabalのライブラリーバージョンは「3.4.0.0」で GHCのバージョンは「8.10.7」であることを確認してください。
{% endhint %}
...
**stake.cert** をブロックプロデューサーノードのcnodeディレクトリにコピーします。
...
**tx.signed**をブロックプロデューサーノードcnodeディレクトリにコピーします。
...
{% hint style="danger" %}
BPにある`vrf.vkey` をエアギャップオフラインマシンのcnodeディレクトリにコピーします。
{% endhint %}
...
**pool.cert**と**deleg.cert**をブロックプロデューサーノードcnodeディレクトリにコピーします。
...
**tx.signed**ブロックプロデューサーノードcnodeディレクトリにコピーします。
...
**stakepoolid.txt**をブロックプロデューサーノードcnodeディレクトリにコピーします。
...
```
```bash
cat allpoosID.txt | grep $(cat stakepoolid.txt)
```
{% endtab %}
{% endtabs %}
...
エアギャップにある **stake.addr**をBPのcnodeフォルダへコピーしてください
{% tabs %}
{% tab title="ブロックプロデューサーノード" %}
```bash
cd $NODE_HOME
...
{% hint style="info" %}
KESーの有効期限が切れると、ブロック生成が出来なくなりますので期限が切れる前に以下の手順で更新してください
{% endhint %}
...
* 5.AG：node.certを生成する (1で算出したKesPeriodを使うこと)
...
#### 18.1-3.BP：KESファイル(kes.skey/kes.vkey)をエアギャップのcnodeディレクトリへコピーする
{% hint style="danger" %}
kes.skey/kes.vkeyをエアギャップオフラインマシンのcnodeディレクトリにコピーします。
{% endhint %}
#### 18.1-4.BPとエアギャップで`kes.vkey`ファイルハッシュを比較する
...
#### 18.1-5.エアギャップ：node.certを生成する (18.1-1で算出したKesPeriodを使うこと)
...
> そこで18.1-1で算出した`startKesPeriod`の数字を入力します
```
echo "入力した数字は$kesです"
```
...
#### 18.1-6.エアギャップ：node.certファイルをBPのcnodeディレクトリへコピーする
...
#### 18.1-7.BPとエアギャップで`node.cert`ファイルハッシュを比較する
...
**pool.cert**と**deleg.cert** をブロックプロデューサーのcnodeディレクトリにコピーします。
...
**tx.signed**ををブロックプロデューサーノードのcnodeディレクトリにコピーします。
...
>
> `rsync -avzhe "ssh -p 12345" myusername@6.1.2.3:/home/myusername/cnode/stake.vkey ./stake.vkey`
...
> `ssh myusername@6.1.2.3 -p 12345`
>
> `rsync -avzhe "ssh -p 12345" ./node.cert myusername@6.1.2.3:/home/myusername/cnode/node.cert`
...
**tx.raw** を **エアギャップオフラインマシン**のcnodeディレクトリにコピーします。
...
**pool.dereg** を **ブロックプロデューサノード**のcnodeディレクトリにコピーします。
...
[18.9 報酬を請求する](https://dev.xstakepool.com/guide-how-to-build-a-haskell-stakepool-node#189-wosuru)  
ステークキー登録料2ADAは、プール登録料が返金された後にステークキー解除処理を行います。（ギルドで別途ご連絡ください）
{% endhint %}

### 💸 19.1 payment.addrから全額引き出す手順
