...
??? note "A.ステータスを確認します"
    === "Stolenの場合"
        ...
        しかしブロックチェーンに採用されるブロックは1スロット1ブロックなので、どのプールのブロックを採用するかを決定する際、ブロック生成時にプールが排出するVRF値が小さい方のブロックを採用する仕様となっております。

        このVRF値は毎回異なるランダムな値を排出しますがステーク量に応じて最大値が変化するため、ステーク量が小さいプールが有利とされています。
